...
namespace Advent_of_Code_2022
{
    internal class Day14
    {
        ...
        public static void Run()
        {
            ...
            double delay = double.Parse(userInput);
            int sleeptime = 1;
            if (delay < 1)
            {
            ...
            while (!finished)
            {
                if (currentSand == null)
                {
                    Point startingPoint = new Point(startingX, 0);
                    if (board.GetPoint(startingPoint) == sandChar)
                    {
                        finished = true;
                    ...
                    {
                        currentSand = new FallingPoint(startingPoint, sandChar, board);
                    }
                }
                int result = currentSand.Fall();
                if (result == SAND_STILL_FALLING)
                {
                    if (reachedFloorAt == 0 && currentSand.loc.Y == board.maxy - 1)
                    {
                ...
                if (stepsTaken % delay == 0)
                {
                    ...
                    board.Render(RENDER_X, RENDER_Y, highlights, trailChar);
        ...
        class Board
        {
            ...
            public Board(string input, char defaultChar)
            {
                ...
                foreach (string line in inputByLine)
                {
                    ...
                    allPaths.Add(new RockPath(points));
                }
                InitializeBoard();
            ...
            public void DrawPoint(int x, int y, char asChar)
            {
                if(charGrid == null)
                {
                    throw new NullReferenceException("Board not yet initialized!");
                }
        ...
        class RockPath
        {
            ...
            public RockPath(List<Point> points)
            {
                this.points = points;
            }
        ...
        class FallingPoint
        {
            ...
            public FallingPoint(Point loc, char sprite, Board board)
            {
                this.loc = loc;
                this.sprite = sprite;
                this.board = board;
                board.DrawPoint(loc, sprite); //Immediately draw us when spawning
            }
            ...
            /// <summary>
            /// Points that we would like to go to, in order of preference: S, SE, SW.
            /// </summary>
            /// <returns>List of points, representing {X,Y+1}, {X+1,Y+1}, {X-1,Y+1}</returns>
            public List<Point> GetCandidates()
            {
                Point down = new Point(loc.X, loc.Y + 1); //Down is up, because the assignment's coordinate system says so
                Point downAndToTheLeft = new Point(loc.X - 1, loc.Y + 1);
                Point downAndToTheRight = new Point(loc.X + 1, loc.Y + 1);
            ...
            public int Fall()
            {
                ...
                for (int i = 0; i < candidates.Count; i++)
                {
                    ...
                    {
                        trajectory.Add(new Point(board.ConvertX(loc.X), board.ConvertY(loc.Y)));
                        board.DrawPoint(loc, board.defaultChar);
