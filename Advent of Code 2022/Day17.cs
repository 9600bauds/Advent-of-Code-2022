...
namespace Advent_of_Code_2022
{
    internal class Day17
    {
        ...
        //static long elephantDemand = 2022;
        static long elephantDemand = 1000000000000;
        ...
        public static void Run()
        {
            ...
            // Basically: The state of the tower will repeat endlessly in a cycle.
            // The length of this cycle is the length of the input (i.e. how many turns until the hot gas jets start repeating)
            // times 5 (i.e. how many pieces until the piece shapes start repeating).
            // There is an initial setup cycle when the game has just started, after that, all following cycles will be exactly the same.
            // So, we simulate this initial cycle, then one cycle more to see the delta of pieces and height in each repeating cycle,
            // and then we can use that information to abstract away however many cycles we want.
            int cycleLength = input.Length * pieceOrder.Count;
            int initialHeight = 0;
            long initialPieces = 0;
            long targetPieces = -1;
            long bonusHeight = -1;
            ...
            while (true)
            {
                ...
                while (pieceIsActive)
                {
                    ...
                    if (rockPushes == cycleLength)
                    {
                        initialHeight = board.highestRockY;
                        initialPieces = piecesAtRest;
                        Console.WriteLine($"In the initial setup cycle, we had {initialPieces} pieces at rest, for a height of {initialHeight}.");
                    }
                    else if (rockPushes == cycleLength * 2)
                    {
                        int heightDelta = board.highestRockY - initialHeight;
                        long piecesDelta = piecesAtRest - initialPieces;
                        Console.WriteLine($"Each repeating cycle is {cycleLength} turns long, adding {piecesDelta} pieces and {heightDelta} height to the tower.");
                        long cyclesNeeded = elephantDemand / piecesDelta - 2; //-2 because we already simulated 2 cycles
                        long skippedPieces = piecesDelta * cyclesNeeded;
                        bonusHeight = cyclesNeeded * heightDelta;
                        Console.WriteLine($"We can abstract away {cyclesNeeded} cycles, skipping {skippedPieces} pieces and adding {bonusHeight} height.");
                        targetPieces = elephantDemand - skippedPieces;
                    }
                }
                ...
                if (targetPieces > 0 && piecesAtRest == targetPieces)
                {
                    GridRenderer.Render(5, 10, board.MakeGrid());
                    Console.WriteLine($"Height of the tower after {elephantDemand} pieces have come to rest: {board.highestRockY + bonusHeight}");
                    return;
                }
            }
        }
