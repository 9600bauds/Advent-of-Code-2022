...
namespace Advent_of_Code_2022
{
    internal class Day22
    {
        ...
        public static void Run()
        {
            ...
            Player player = new(board.GetStarterPoint(), DIR_RIGHT, instructions, board, '@');

            player.ExecuteInstructions();
            GridRenderer.Render(0, 10, board.grid);
            Console.WriteLine($"Final password for Part 1: {player.GetFinalSum()}");

            CubeBoard cube = new(gridBackup, cubeEdges);
            Player cubePlayer = new(cube.GetStarterPoint(), DIR_RIGHT, instructions, cube, '@');

            //cubePlayer.AssumeControl(); //Just for testing

            cubePlayer.ExecuteInstructions();
        ...
        class Player
        {
            ...
            public Player(Point loc, int dir, List<string> instructions, Board board, char sprite)
            {
                ...
                this.board = board;
                this.sprite = sprite;
                board.SetPixel(loc, sprite); //Immediately draw us upon spawning
            }

            /// <summary>
            /// Score calculation as defined by the website.
            /// </summary>
            ...
            public bool Move(Point p)
            {
                ...
                {
                    GridRenderer.Render(0, 10, board.grid); //for testing if warp logic works correctly
                }*/
            ...
            /// <summary>
            /// Makes X amount of right-hand turns. Allows for negative numbers to turn anticlockwise. Also changes our sprite to the direction we're facing.
            /// </summary>
            ...
            /// <summary>
            /// Tries to move forwards 1 step.
            /// </summary>
            /// <returns>True if we moved, False if we couldn't move (wall in front of us).</returns>
            public bool Step()
            {
                (Point nextPoint, int dirOffset) = board.GetNextPoint(loc, dir);
            ...
            /// <summary>
            /// Execute all our walking instructions.
            /// </summary>
            ...
            /// <summary>
            /// Lets the user take manual control and walk around with the arrow keys. Used for manually testing the program logic.
            /// </summary>
        ...
        class Board
        {
            ...
            public Point GetInBounds(Point p)
            {
                int X = p.X;
                X %= width;
                if (X < 0) { X = width - 1; }
            ...
            /// <summary>
            /// If we were to take 1 step forwards, where would we end up, and where would we be facing?
            /// This can be in a very different spot and facing a very different direction, in Part 2 of the puzzle.
            /// This is a virtual method and gets overwritten by the cube-type board.
            /// </summary>
            /// <param name="start">Our current location</param>
            /// <param name="dir">Our current direction</param>
            /// <returns>A tuple containing our next direction, and our direction afterwards.</returns>
            public virtual (Point, int) GetNextPoint(Point start, int dir)
            {
                Point curr = new(start.X, start.Y);
                int dirDelta = 0;
                curr = ShiftPoint(curr, dir);
                if (IsOutOfBounds(curr))
                {
                    curr = GetInBounds(curr);
                }
                while (GetPixel(curr) == ' ')
                {
            ...
            public static Point ShiftPoint(Point curr, int dir)
            {
                switch (dir)
                {
                    case DIR_RIGHT:
        ...
        class CubeBoard : Board
        {
            ...
            public (Point, int) CrossCubeEdge(Point p, int dir)
            {
                ...
                Trig.Point3D meIn3d = new(p.X, p.Y, 0);
        ...
        public static (char[,] grid, List<string> code, List<Edge> cubeEdges) ProcessInput(string input)
        {
            ...
            int blocksize = Math.Max(width, height) / 4; //How big is each face of the cube? Well, an unfolded cube can only be at most 4 across in one direction, so we divide the greatest axis by 4
            List<Face> faces = new List<Face>();
            for (int i = height / blocksize - 1; i >= 0; i--)
            {
                for (int j = 0; j < width / blocksize; j++)
                {
                    ...
                    if (grid[x, y] != ' ')
                    {
                        ...
                        Trig.Point3D point2 = new Trig.Point3D(x, y + blocksize, 0);
                        Trig.Point3D point3 = new Trig.Point3D(x + blocksize, y + blocksize, 0);
                        Trig.Point3D point4 = new Trig.Point3D(x + blocksize, y, 0);
